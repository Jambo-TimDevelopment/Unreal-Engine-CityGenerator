	  ...
	  CachedLocalBounds(ForceInit)
{
	...
	static ConstructorHelpers::FClassFinder<AStreetMapSplineBuilding> BPStreetMapSplineBuilding(TEXT("/StreetMap/ProceduralBuildingGenerator/Blueprints/Procedural_Building_Generator_BP"));
	StreetMapSplineBuildingClass = BPStreetMapSplineBuilding.Class;

	// set default controller to our Blueprinted controller
	static ConstructorHelpers::FClassFinder<AStreetMapSplineRoads> BPStreetMapSplineRoadClass(TEXT("/StreetMap/SnappyRoads/Blueprints/BP_SnappyRoad"));
	StreetMapSplineRoadClass = BPStreetMapSplineRoadClass.Class;

	// set default controller to our Blueprinted controller
	static ConstructorHelpers::FClassFinder<AStreetMapPointNode> BPStreetMapPointNodeClass(TEXT("/StreetMap/SnappyRoads/Blueprints/BP_Node"));
	StreetMapPointNodeClass = BPStreetMapPointNodeClass.Class;
}
...
void UStreetMapComponent::GenerateBuildings()
{
	CachedLocalBounds = FBox(ForceInit);
	Vertices.Reset();
	Indices.Reset();

	if (StreetMap == nullptr) { return; }
	FBox3f MeshBoundingBox;
	MeshBoundingBox.Init();

	TArray<FStreetMapBuilding> Buildings = StreetMap->GetBuildings();
	...
	for (int32 BuildingIndex = 0; BuildingIndex < Buildings.Num(); ++BuildingIndex)
	{
		FStreetMapBuilding Building = Buildings[BuildingIndex];
		...
		AStreetMapSplineBuilding* PlacementObject = World->SpawnActor<AStreetMapSplineBuilding>(StreetMapSplineBuildingClass, SpawnParameters);

		PlacementObject->SetOwner(GetOwner());
		PlacementObject->SetActorLocation(GetOwner()->GetActorLocation());
		PlacementObject->SetStreetMapBuilding(Building);
		PlacementObject->UserConstructionScript();
	}
}

void UStreetMapComponent::GenerateRoads()
{
	/////////////////////////////////////////////////////////
	// Visual tweakables for generated Street Map mesh
	//
	...
	if (StreetMap != nullptr)
	{
		...
		{
			FStreetMapRoad Road = Roads[RoadIndex];
			...
			FActorSpawnParameters SpawnParameters;
			SpawnParameters.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
			AStreetMapSplineRoads* PlacementObject = World->SpawnActor<AStreetMapSplineRoads>(StreetMapSplineRoadClass, SpawnParameters);

			PlacementObject->SetOwner(GetOwner());
			PlacementObject->SetActorLocation(GetOwner()->GetActorLocation());
			PlacementObject->SetStreetMapRoad(Road);
			PlacementObject->UserConstructionScript();

			RoadActors.Add(PlacementObject);
		}


		for (int32 NodeIndex = 0; NodeIndex < Nodes.Num(); ++NodeIndex)
		{
			FStreetMapNode Node = Nodes[NodeIndex];
